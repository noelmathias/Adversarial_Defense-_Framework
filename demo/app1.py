...
with col_upload:
    ...
    if uploaded is not None:
        ...
        st.image(
            tensor_to_display(tensor),
            caption="Clean input (32×32, upscaled)",
            width=220,
        )
...
if "input_tensor" not in st.session_state:
    ...
    if run_btn:
        msg = ("Generating adversarial example..."
               if attack_type == "fgsm"
               else "Running PGD (20 iterations)...")
        with st.spinner(msg):
        ...
        for k in ["detection","clean_detection","risk_score","risk_level",
                  "decision","recovery"]:
            st.session_state.pop(k, None)
    ...
    if "adv_tensor" in st.session_state:
        ...
        with col_chart:
            ...
            x, width = np.arange(len(CIFAR10_CLASSES)), 0.38
            fig, ax = plt.subplots(figsize=(5, 3.5))
            ax.bar(x-width/2, clean_probs, width, label="Clean",
                   color="steelblue", alpha=0.85, edgecolor="black", linewidth=0.4)
            ax.bar(x+width/2, adv_probs,   width, label=atk_used,
...
if "adv_tensor" not in st.session_state:
    ...
    if st.button("🔎 Analyze Input", use_container_width=False):
        with st.spinner("Computing detection signals..."):
    ...
    if "detection" in st.session_state:
        ...
        with s3:
            ...
            st.metric(
                "vs Clean",
                f"{adv_sig['conf_drop']:.3f}",
                delta=None,
            )
            st.caption("Relative confidence loss.")

        with s4:
            st.markdown("##### Grad Mag")
            grad_clean = clean_sig.get("grad_magnitude", 0.0)
            grad_adv   = adv_sig.get("grad_magnitude", 0.0)
            st.metric(
                "Normalized",
                f"{grad_adv:.3f}",
                delta=f"{grad_adv - grad_clean:+.3f} vs clean",
                delta_color="inverse",
            )
            st.caption("High = catches confident-wrong PGD.")
        ...
        with chart_col1:
            ...
            clean_vals = [
                ...
                clean_sig.get("grad_magnitude", 0.0),
            ...
            adv_vals = [
                ...
                adv_sig.get("grad_magnitude", 0.0),
                adv_sig["risk_score"],
            ]
        ...
        with chart_col2:
            ...
            ax.set_ylim(0, 1)
            ax.axis("off")
            ax.barh(0.5, 1.0,  height=0.35, color="#333333",
                    left=0.0,  align="center")
            ax.barh(0.5, RISK_LOW_THRESHOLD,
                    height=0.35, color="#52e07a",
                    left=0.0,  alpha=0.5, align="center")
            ax.barh(0.5, RISK_MEDIUM_THRESHOLD - RISK_LOW_THRESHOLD,
                    height=0.35, color="#e0b852",
                    left=RISK_LOW_THRESHOLD, alpha=0.5, align="center")
            ax.barh(0.5, 1.0 - RISK_MEDIUM_THRESHOLD,
                    height=0.35, color="#e05252",
                    left=RISK_MEDIUM_THRESHOLD, alpha=0.5, align="center")
            ax.plot([risk, risk], [0.25, 0.75],
                    color="white", linewidth=3, zorder=5)
            ax.plot(risk, 0.75, "v", color="white",
...
if "risk_score" not in st.session_state:
    ...
    st.session_state["decision"] = {
        "tier": tier, "config": config, "reason": reason,
    }
    ...
    st.markdown(
        ...
        <div style="padding:18px 28px;border-radius:10px;
            background:{bg};border-left:6px solid {fg};
            margin-bottom:16px;">
        ...
        with col:
            st.markdown(
                ...
                <div style="padding:14px;border-radius:8px;
                    background:{b_bg};border:2px solid {b_color};
                    min-height:180px;">
    ...
    with st.expander("🔬 Decision Trace (Research View)"):
        ...
        st.info(
            ...
            "from reward signals over time. The interface above stays unchanged."
        )
...
if "decision" not in st.session_state:
    ...
    if st.button(f"🛡️ Apply {config['label']}", use_container_width=False):
        ...
        rec_status = compute_recovery_status(
            ...
            def_label=def_label,
            def_conf=def_conf,
            pred_mismatch=st.session_state.get(
                "detection", {}
            ).get("pred_mismatch", 0),
        )
    ...
    if "recovery" in st.session_state:
        ...
        if recovered:
            st.success(
                ...
                f"{rec_status['conf_recovery_pct']:.1f}% of gap closed."
            )
            ...
            st.warning(
                ...
                f"**{def_label.upper()}**, clean was **{clean_label.upper()}**. "
                f"Defense reduced adversarial effect but label not restored."
            )

        # ── Three-stage image strip ────────────────────
        st.markdown("**Input Progression: Clean → Attack → Defense**")
        img_c1, arr1, img_c2, arr2, img_c3 = st.columns([1, 0.1, 1, 0.1, 1])

        with img_c1:
            st.image(tensor_to_display(st.session_state["input_tensor"]),
                     caption=f"① Clean\n{clean_label.upper()} ({clean_conf*100:.0f}%)",
                     width=160)
        with arr1:
            st.markdown(
                "<div style='font-size:26px;margin-top:65px;"
                "text-align:center;color:#666;'>→</div>",
                unsafe_allow_html=True,
            )
        with img_c2:
            atk_used = st.session_state.get("attack_type","fgsm").upper()
            ...
            st.image(tensor_to_display(st.session_state["adv_tensor"]),
                     caption=f"② {atk_used} (ε={eps_used})\n"
                             f"{adv_label.upper()} ({adv_conf*100:.0f}%)",
                     width=160)
        with arr2:
            st.markdown(
                "<div style='font-size:26px;margin-top:65px;"
        ...
        with img_c3:
            st.image(tensor_to_display(rec["defended_img"]),
                     caption=f"③ Defended ({tier.capitalize()})\n"
                             f"{def_label.upper()} ({def_conf*100:.0f}%)",
                     width=160)
        ...
        with m1:
            st.metric("Clean", f"{clean_conf*100:.1f}%")
        with m2:
            st.metric("After Attack", f"{adv_conf*100:.1f}%",
                      delta=f"{(adv_conf-clean_conf)*100:.1f}%",
                      delta_color="inverse")
        with m3:
            st.metric("After Defense", f"{def_conf*100:.1f}%",
                      delta=f"{(def_conf-adv_conf)*100:.1f}% recovered",
                      delta_color="normal")
        with m4:
        ...
        ax.axvline(true_idx, color="gold", linewidth=2,
                   linestyle="--", alpha=0.6, label="True class")
        ax.set_xticks(x)
        ...
        ax.grid(True, alpha=0.3, axis="y")
        ax.set_title("Prediction Distribution: Clean vs. Adversarial vs. Defended",
                     fontsize=11)
        plt.tight_layout()
        st.pyplot(fig)
        plt.close()
        ...
        with st.expander("🔬 Recovery Trace (Research View)"):
            ...
            st.markdown(
                ...
                | After attack  | {adv_label.upper()}   | {adv_conf*100:.1f}%  |
                | After defense | {def_label.upper()}   | {def_conf*100:.1f}%  |

                **Defense applied:** `{tier}` — `{config['params']}`
                **Label recovered:** `{'Yes ✓' if recovered else 'No ✗'}`
                **Confidence recovery:** `{recovery_display}`
                """
            )
...
if "recovery" not in st.session_state:
    ...
    st.markdown(
        ...
        <div style="display:flex;gap:0;margin-bottom:20px;
            ...
            <div style="flex:1;padding:14px 18px;
                background:#1a2a3a;text-align:center;">
                <div style="font-size:11px;color:#888;margin-bottom:4px;">
                    ① CLEAN
                </div>
                <div style="font-size:15px;font-weight:600;color:#5aabff;">
                    {clean_label.upper()}
                </div>
            ...
            <div style="flex:1;padding:14px 18px;
                background:#3a1a1a;text-align:center;">
                <div style="font-size:11px;color:#888;margin-bottom:4px;">
                    ② {atk_used} (ε={eps_used})
                </div>
                <div style="font-size:15px;font-weight:600;color:#ff6b6b;">
                    {adv_label.upper()}
                </div>
                <div style="font-size:12px;color:#aaa;">
                    {adv_conf*100:.1f}%
            ...
            <div style="width:1px;background:#333;"></div>
            <div style="flex:1;padding:14px 18px;
                background:#2a2a1a;text-align:center;">
                <div style="font-size:11px;color:#888;margin-bottom:4px;">
                    ③ DETECTION
                </div>
            ...
            <div style="width:1px;background:#333;"></div>
            <div style="flex:1;padding:14px 18px;
                background:#1a2a1a;text-align:center;">
                <div style="font-size:11px;color:#888;margin-bottom:4px;">
                    ④ DECISION
                </div>
                <div style="font-size:15px;font-weight:600;
                    color:{tier_clr[tier]};">
                    {tier_icons[tier]} {tier.upper()}
                </div>
                <div style="font-size:12px;color:#aaa;">
                    {config['params']}
                </div>
            ...
            <div style="width:1px;background:#333;"></div>
            <div style="flex:1;padding:14px 18px;
                background:{'#0f2d12' if recovered else '#2d1a0a'};
                text-align:center;">
                <div style="font-size:11px;color:#888;margin-bottom:4px;">
                    ⑤ DEFENSE
                </div>
    ...
    card_data = [
        (
            ...
            "① Clean Input", clean_label, clean_conf, clean_probs,
    ...
    for col, (tensor, title, lbl, conf, probs,
              bar_c, bg, fg, badge) in zip(
        ...
        with col:
            st.markdown(
                ...
                <div style="background:{bg};border-radius:10px;
                    padding:14px;margin-bottom:10px;border:1px solid #333;">
            ...
            ax.barh(CIFAR10_CLASSES, probs, color=bar_c,
                    alpha=0.85, edgecolor="black", linewidth=0.3)
            # Highlight top predicted
            ax.barh([CIFAR10_CLASSES[int(probs.argmax())]],
                    [probs.max()], color=fg, alpha=1.0,
                    edgecolor="black", linewidth=0.3)
            # Mark true class in gold
            true_idx = CIFAR10_CLASSES.index(clean_label)
            ax.barh([CIFAR10_CLASSES[true_idx]], [probs[true_idx]],
    ...
    # ── Overlaid three-way chart ───────────────────────
    st.markdown("**Probability Shift Across Pipeline Stages**")
    x, w = np.arange(len(CIFAR10_CLASSES)), 0.26
    ...
    ax.bar(x+w, def_probs,   w, label=f"③ Defended ({tier})",
           color="mediumseagreen", alpha=0.85, edgecolor="black", linewidth=0.3)
    ax.axvline(true_idx, color="gold", linewidth=2,
               linestyle="--", alpha=0.7, label="True class")
    ax.set_xticks(x)
    ax.set_xticklabels(CIFAR10_CLASSES, rotation=45, ha="right", fontsize=9)
    ax.set_ylabel("Probability", fontsize=10)
    ax.set_ylim(0, 1)
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3, axis="y")
    ax.set_title("Full Pipeline: Probability Distribution at Each Stage",
                 fontsize=12)
    plt.tight_layout()
    st.pyplot(fig)
    plt.close()
    ...
    with tbl_c2:
        grad_clean = st.session_state.get(
            "clean_detection", {}
        ).get("grad_magnitude", 0.0)
        st.markdown(
            f"""
            ...
            | Grad magnitude   | {grad_clean:.3f}  | {det.get('grad_magnitude',0):.3f} |
    ...
    with st.expander("📋 Export Pipeline Result"):
        export = {
            "attack": {
                "type":    st.session_state.get("attack_type","fgsm"),
                "epsilon": eps_used,
            },
            "clean": {
                "label":      clean_label,
            ...
            "detection": {
                ...
                "grad_magnitude": round(det.get("grad_magnitude", 0.0), 4),
            ...
            "defended": {
                ...
                "recovery_note": (
                    ...
                    or ("Successful" if recovered
                        else "Partial — label not restored")
                ),
            },
        }
...
st.caption(
    ...
    f"Attacks: FGSM / PGD · "
    f"Detection: 5-signal (conf, entropy, conf_drop, grad_mag, mismatch) · "
    f"Defense: Light / Medium / Strong (randomized smoothing) · "
    f"Phase 2 in progress"
)